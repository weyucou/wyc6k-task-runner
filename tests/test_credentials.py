...
class TestFetchSecret:
    def test_returns_plain_string(self) -> None:
        ...
        with patch("marvin.credentials._get_secrets_client", return_value=mock_client):
    ...
    def test_raises_runtime_error_on_fetch_failure(self) -> None:
        ...
        with patch("marvin.credentials._get_secrets_client", return_value=mock_client):
            with pytest.raises(RuntimeError, match="Failed to fetch secret ARN="):
...
class TestCredentialResolver:
    def test_github_token_arn_sets_env_var(self, monkeypatch) -> None:
        monkeypatch.delenv("GITHUB_TOKEN", raising=False)
        envelope = _make_envelope(github_token_arn="arn:aws:secretsmanager:ap-northeast-1:123:secret:gh")
        mock_client = MagicMock()
    ...
    def test_anthropic_key_arn_sets_agent_api_key(self) -> None:
        envelope = _make_envelope(anthropic_api_key_arn="arn:aws:secretsmanager:ap-northeast-1:123:secret:ak")
        mock_client = MagicMock()
    ...
    def test_fallback_to_env_github_token(self, monkeypatch) -> None:
        ...
        envelope = _make_envelope()  # no ARN fields
        CredentialResolver().resolve(envelope)
    ...
    def test_raises_before_processing_on_arn_failure(self) -> None:
        envelope = _make_envelope(github_token_arn="arn:aws:secretsmanager:ap-northeast-1:123:secret:bad")
        mock_client = MagicMock()
    ...
    def test_no_arns_no_env_vars_resolves_none(self, monkeypatch) -> None:
        monkeypatch.delenv("GITHUB_TOKEN", raising=False)
    ...
    def test_does_not_log_secret_value(self, monkeypatch, caplog) -> None:
        ...
        envelope = _make_envelope(
            github_token_arn="arn:aws:secretsmanager:ap-northeast-1:123:secret:gh",
            anthropic_api_key_arn="arn:aws:secretsmanager:ap-northeast-1:123:secret:ak",
        )
        mock_client = MagicMock()
